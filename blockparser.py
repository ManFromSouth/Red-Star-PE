...
class BlockParser:
    ...
    # заполняет данные о блоках
    def load_lines(self, lines_list):
        self.__result = lines_list.copy()

    # чистит спсики блоков
    def clear_results(self):
        self.__result = list()

    def replace_block(self, line_num, block_num, block):
        """
        Заменяет указанный блок переданным
        :param line_num: номер строки
        :param block_num: номер блока в строке
        :param block: замена для блока
        """
        self.__result[line_num][block_num] = block.copy()

    def load_array(self, array):
        """
        Загружает байтовый массив через parse_line, попутно, разбив его на строки
        :param array:
        :return:
        """
        self.clear_results()
